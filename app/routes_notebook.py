...
# # process PATTIE outline html and prepare for Jupyter Notebook
# @app.route('/notebook', methods=['POST'])
# def notebook():
#     if request.method == 'POST':
#         # clean and parse outline
#         transcript = request.get_json()
#         # print(transcript)
#         # serialize transcript to JSON
#         json_object = json.dumps(transcript, indent=4)
#         # Writing to sample.json
#         with open("PATTIE_Session/PATTIE_outline.json", "w") as outfile:
#             outfile.write(json_object)
#         # execute the Notebook
#         import subprocess
#         subprocess.run(
#             'cd PATTIE_Session/; jupyter lab --core-mode PATTIE_Notebook.ipynb', shell=True)

#     return '', 200

# @app.route('/pattie_notebook')
# def home_notebook():
...
#     return render_template('index_notebook.html', field=field, uploaded_files=uploaded_files,
#                            start_date=yesterday, end_date=today)

# @app.route('/cluster_notebook', methods=['POST'])
# def cluster_notebook():
...
#     return render_template("results_notebook.html",
#                            q=q,
#                            error=error,
#                            keywords=keywords,
...
# @app.route('/_re_cluster_notebook')
# def re_cluster_notebook():
...
#     if dataset == "NYTIMES":
#         membership, cluster_centers, cluster_desc, coordinates, error = \
#             vl.kmeans(docs, keywords, n_components=7, k=num_cls,
#                       n_desc=15)
#     else:
#         print(dataset)  # newly add
#         membership, cluster_centers, cluster_desc, coordinates, error = \
#             vl.kmeans(docs, keywords, n_components=10, k=num_cls,
#                       n_desc=25)
...
#             references = []
#             for bib in id2members[cluster]:
#                 references.append(bibs_new[bib])
#             for paper in references:
#                 if type(paper['author']) != str and None in paper['author']:
#                     paper['author'] = [i for i in paper['author'] if i]
#             bibliography = {'concepts': concepts, 'references': references}
...
#     # Store session data
#     session['membership_' + str(state+1)] = membership
#     session['cluster_desc_' + str(state+1)] = cluster_desc
#     session['xy_' + str(state+1)] = cluster_centers
#     session['hue_' + str(state+1)] = hue
#     session['satr_' + str(state+1)] = satr
#     session['chosen_' + str(state)] = ids
#     session['edges_' + str(state+1)] = edges
#     # session['sources_' + str(state+1)] = sources
